...
GRANT USAGE ON CORTEX SEARCH SERVICE sales_conversation_search TO ROLE <Your Role>;

GRANT OWNERSHIP ON DATABASE SALES_INTELLIGENCE TO ROLE <Your Role>; 

GRANT OPERATE ON WAREHOUSE sales_intelligence_wh TO ROLE <Your Role> WITH GRANT OPTION;

GRANT CREATE AGENT ON SCHEMA snowflake_intelligence.agents TO ROLE  <Your Role>; -- ability for role to create an agent

GRANT ROLE sales_intelligence_rl to user <username>; -- assinging role to user

GRANT DATABASE ROLE SNOWFLAKE.CORTEX_USER TO ROLE  <Your Role>;

GRANT ROLE cortex_user_role TO USER <username>;

GRANT USAGE ON SCHEMA sales_intelligence.data TO ROLE  <Your Role>;
GRANT USAGE ON DATABASE sales_intelligence TO ROLE  <Your Role>;
GRANT USAGE ON WAREHOUSE sales_intelligence_wh TO ROLE  <Your Role>;
GRANT CREATE STREAMLIT ON SCHEMA sales_intelligence.data TO ROLE  <Your Role>;
GRANT CREATE STAGE ON SCHEMA sales_intelligence.data TO ROLE  <Your Role>;
GRANT SELECT ON ALL TABLES IN SCHEMA sales_intelligence.data to ROLE  <Your Role>;

ALTER USER <username> SET DEFAULT_ROLE= <Your Role>;


-- External API / Tool call functions

-- 1. Allow HTTPS access to Alpha Vantage API - Stock price
CREATE OR REPLACE NETWORK RULE alphavantage_rule
  MODE = EGRESS
  TYPE = HOST_PORT
  VALUE_LIST = ('www.alphavantage.co:443');

-- 2. Create an external access integration for Alpha Vantage
CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION alphavantage_eai
  ALLOWED_NETWORK_RULES = (alphavantage_rule)
  ENABLED = TRUE;

-- 3. Grant usage to your role
GRANT USAGE ON INTEGRATION alphavantage_eai TO ROLE <Your Role>;



CREATE OR REPLACE FUNCTION alpha_monthly_history(ticker STRING, months INT)
RETURNS VARIANT
LANGUAGE PYTHON
RUNTIME_VERSION = '3.10'
PACKAGES = ('requests', 'pandas')
EXTERNAL_ACCESS_INTEGRATIONS = (alphavantage_eai)
HANDLER = 'handler'
AS
$$
import requests, pandas as pd

def handler(ticker, months):
    url = "https://www.alphavantage.co/query"
    params = {
        "function": "TIME_SERIES_MONTHLY",
        "symbol": ticker,
        "apikey": "AQWQFF1ILPCKS2ED"
    }
    try:
        ...
        if not ts:
            return {"ticker": ticker, "history_monthly": [], "raw": data}
        # Map Alpha Vantage keys to simple names
        ...
        return {"ticker": ticker, "history_monthly": out}
...
SELECT alpha_monthly_history('HLTT', 12);

-- Test send email. we will be using this in the agent to send emails to the user
...
CREATE OR REPLACE NOTIFICATION INTEGRATION my_email_int
  TYPE = EMAIL
  ENABLED = TRUE
  ALLOWED_RECIPIENTS = (<emails comma separated>);

-- use this procedure to send html email, we will be using this in the agent to send emails to the user
CREATE OR REPLACE PROCEDURE send_html_email(
    html_body VARCHAR,
    subject VARCHAR,
    to_email VARCHAR
)
...
// Use uppercase variable names for parameters
var cleanHtml = escapeSingleQuotes(HTML_BODY);
var toEmail = TO_EMAIL;
var subj = SUBJECT;

var send_stmt = `CALL SYSTEM$SEND_EMAIL(
    'my_email_int', 
    '${toEmail}', 
    '${subj}', 
    '${cleanHtml}', 
...
try {
    snowflake.execute({sqlText: send_stmt});
    return 'Email sent!';
} catch (err) {
    return 'FAILED: ' + err;
}
...
--- Agents Chat History Management Table
create or replace TABLE SALES_INTELLIGENCE.DATA.CONVERSATION_TRACKING (
	ID NUMBER(38,0) NOT NULL autoincrement start 1 increment 1 noorder,
	THREAD_ID NUMBER(38,0) NOT NULL,
	THREAD_NAME VARCHAR(255),
	PARENT_MESSAGE_ID NUMBER(38,0) NOT NULL,
	USER_ID VARCHAR(255),
